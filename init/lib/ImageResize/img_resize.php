...
function img_resize($src, $dest, $width, $height,  $quality=100, $rgb=0xFFFFFF, $fon=0, $r=0)
{
...
//		imagecopy($idest, $im_logo, 0, 0, 0, 0, 150, 25);
//	}
	if($r) {
	/**
 * ??????????????? :)
...
foreach ($positions as $pos) {
    imagecopyresampled($corner, $idest, $pos[0], $pos[1], $pos[2], $pos[3], $rs_radius, $rs_radius, $radius, $radius);
}
...
foreach ($positions as $i => $pos) {
    imagecopyresampled($idest, $corner, $pos[2], $pos[3], $pos[0], $pos[1], $radius, $radius, $rs_radius, $rs_radius);
}
	imagepng($idest, $dest);
	} else {
