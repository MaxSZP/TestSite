...
for ($i = 0; $i < $files_count; $i++) {	
	...
	//		if ($_POST['orient']==0) {
			...
			if (is_dir("../../konkurs/".$_POST['id']."/")) {
				
			if (file_exists("../../konkurs/".$_POST['id']."/" .$new_name)){
				$name1 =  str_replace(".jpg","", $new_name);

				$new_name1 = $name1."_1".".jpg";
				if (file_exists("../../konkurs/".$_POST['id']."/" .$new_name1)){
				$new_name1 = $name1.filemtime("../../konkurs/".$_POST['id']."/".$new_name1).".jpg";
				}
		//		move_uploaded_file($new_tmp_name, "../../".$_POST['dir']."/gallery/".$_POST['desing']."/" . $new_name1);
			...
			//	img_resize($new_tmp_name, "../../foto/".$_POST['dir']."/" . $new_name1, $width, $height,  90, 0x002c59, 0);
				img_resize($new_tmp_name, "../../konkurs/".$_POST['id']."/" . $new_name1, $thumb_width, $thumb_height,  90, 0x002c59, 0);
				$query_update = "UPDATE `contest`  SET  `thumb`='$new_name1', `del`='0' WHERE `id`='".$_POST['id']."'";
				...
				$msg .=  $new_name;
			//	img_resize($new_tmp_name, "../../konkurs/".$_POST['id']."/" . $new_name, $width, $height,  90, 0x002c59, 0);
				img_resize($new_tmp_name, "../../konkurs/".$_POST['id']."/" .$new_name, $thumb_width, $thumb_height,  90, 0x002c59, 0);
				$query_update = "UPDATE `contest`  SET  `thumb`='$new_name', `del`='0' WHERE `id`='".$_POST['id']."'";
				$update= mysql_query($query_update, $base) or die(mysql_error()); 

			}
			} else {

			mkdir("../../konkurs/".$_POST['id']."/", 0777);
    ...
    <?php Header("Location: ../base.php?id=10&dir=".$_POST['dir']."");?>
